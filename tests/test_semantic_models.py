...
def graph(quote="name maxLength: 128 characters", scope="system-a"):
    ...
    entity = Entity(entity_id="name", entity_type="BusinessField", display_name="name",
                    canonical_name="name", scope=scope, source_document_ids=["doc"])
    evidence = Evidence(evidence_id="ev", document_id="doc", section_id="sec", chunk_id="chunk",
                        quote=quote, evidence_type="text", supports=[],
                        source_location=SourceLocation(file="design.md", line_start=1, line_end=1))
    relation = Relation(relation_id="rel", relation_type="DEFINES", source_id="api",
                        target_id="name", evidence_ids=["ev"], status="confirmed")
...
def test_all_contracts_round_trip():
    ...
    operation = operation_from_atom(ChangeAtom(
        atom_id="op", change_id="change", target_terms=["name"], operation="change_constraint",
        property="length", before="128", after="256", before_unit="characters",
        after_unit="characters", scope="system-a",
    ))
    identity = IdentityAssertion(left_id="a", right_id="b", scope="system-a",
                                 relation="same", evidence_ids=["ev"])
    models = [anchor, mentions[0], assertions[0], operation, identity, operation.after]
    assert {type(m) for m in models} == {
        SourceAnchor, Mention, SpecAssertion, ChangeOperation, IdentityAssertion, LengthValue,
    }
    for model in models:
...
@pytest.mark.parametrize("quote", [
    "name appears on page 128", "name; unrelated maxLength: 128 characters",
    "surname maxLength: 128 characters", "name maxLength: 12 maxLength: 128",
])
...
def test_table_and_conditions_are_preserved():
    _, assertions = extract_assertions(*graph(
        "| field | maxLength |\n| --- | --- |\n| name | 128 bytes |"
    ))
...
def test_multiple_changes_and_units_round_trip(tmp_path: Path):
    ...
    path.write_text("# 変更\nプロジェクト名の最大長を128文字から256文字へ変更。\n"
                    "説明の最大長を32バイトから64バイトへ変更", encoding="utf-8")
