...
class ApplicationService:
    """UI-independent facade shared by the CLI, Web API, and MCP server."""

    project: Project

    @property
    def store(self) -> LocalStore:
        return store_for(self.project)

    def overview(self) -> dict[str, Any]:
        return project_overview(self.project)

    def graph(self, **params: Any) -> dict[str, Any]:
        return graph_data(self.project, **params)

    def evidence(self, evidence_id: str | None = None) -> dict[str, Any]:
        return evidence_data(self.project, evidence_id)

    def design_documents(self, **params: Any) -> dict[str, Any]:
        return design_documents_data(self.project, **params)

    def sources(self) -> dict[str, Any]:
        return source_library_data(self.project)

    def report(self) -> dict[str, Any]:
        return report_data(self.project)

    def reviews(self) -> dict[str, Any]:
        return review_queue_data(self.project)

    def aliases(self) -> dict[str, Any]:
        return aliases_data(self.project)

    def dirty_excel(self) -> dict[str, Any]:
        return dirty_excel_data(self.project)

    def impacts(self, change_id: str | None = None) -> list[dict[str, Any]]:
        return impact_decisions_data(self.project, change_id)

    def integrations(self) -> dict[str, Any]:
        return integration_data(self.project)

    def preview(self, action: str, params: dict[str, Any]) -> dict[str, Any]:
        return external_preview(self.project, action, params)

    def execute(self, action: str, params: dict[str, Any]) -> dict[str, Any]:
        return execute(self.project, action, params)
    ...
    def mutate(
        ...
        *,
        ...
        return MutationCoordinator(self.store).run(
            idempotency_key=idempotency_key,
            action=action,
            params=params,
            operation=lambda: execute(self.project, action, params),
        )

    def tool(self, tool: str, params: dict[str, Any]) -> str:
        return tool_result(self.project, tool, params)

    def call(self, operation: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        """Run a legacy CLI domain operation against this service's project store."""
        return operation(self.store, *args, **kwargs)
...
def execute(
    ...
    params: dict[str, Any],
    *,
    _dependencies: Mapping[str, Callable[..., Any]] | None = None,
) -> dict[str, Any]:
    ...
    dependencies = _dependencies or {}
    evaluate_dataset_fn = dependencies.get("evaluate_dataset", evaluate_dataset)
    release_validate_fn = dependencies.get("release_validate", release_validate)
    if action == "init":
        store.init()
